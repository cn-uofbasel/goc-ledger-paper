...
\subsubsection{Strict Sequences of Account Operations Maintain a Non-Negative Balance}
...
\assume{\begin{pfenum}
	\item $L \in \mathds{L}$
	\item $\forall_{(id,b) \in \texttt{balances}(L^0)} b \geq 0$
	\item $n \in \mathds{N}$
\end{pfenum}}
\define{\begin{pfenum}
		\item $\textit{op}(L) \defeq \begin{cases}
	        			\texttt{initialize$_\mathds{A}$}(id) :  id \in \mathds{I} \wedge id ~\text{is globally unique}  \\
...
\subsubsection{Concurrent Sequences of Operations on Different Accounts Always Result in Non-Negative Balances}

\assume{\begin{pfenum}
	\item $L^0 \in \mathds{L}$
	\item $\forall_{(id,b) \in \texttt{balances}(L)} b \geq 0$
	\item $n \in \mathds{N} \wedge n \geq 1$
\end{pfenum}}
\define{\begin{pfenum}
	\item $\textit{Conc}(L,n) \defeq \begin{cases}
			\textit{Seq}(L,n) \\
			\textit{Seq}(L',i) : i,j \in \mathds{N} \wedge i + j = n \wedge j \geq 1 \wedge L' = \textit{Conc}(L,j)  \\ 
			L' \sqcup_\mathds{L} L'' : 
		 	 i = n-1 \wedge 0 \leq j \leq i \wedge L' = \textit{Conc}(L,j) \wedge L'' = \textit{Conc}(L,i)
	           \end{cases}$
\end{pfenum}}
...
\begin{proof}
	...
	\pf~ 	All operations in \textit{Ops} only modify the state of a single account. This is true even for transferring tokens because the sending (\texttt{giveTo}) is decoupled from the reception (\texttt{ackFrom}). 
\end{proof}
